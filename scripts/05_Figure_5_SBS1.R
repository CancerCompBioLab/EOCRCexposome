...
# Fig5a: SBS1-score distribution over the 3 age categories
library(ggplot2)
...
## Fig5b MRS picloram vs SBS 1 score threshold of 60 
...
## Fig5c Chronological age vs SBS1 
...
#### Can we use TMB instead of SBS1?

library(maftools)

## Download mutation data from TCGA using TCGAbiolinks

maf = "COAD_TCGA_snv.maf" 
laml = read.maf(maf = maf)

filtered_maf <- subsetMaf(laml, query = "Variant_Classification != 'Silent'")
total_mutations <- nrow(filtered_maf@data)
samsum <- getSampleSummary(filtered_maf)
coding_region_size_mb <- 38  # Standard exome size
samsum$tmb <-samsum$total / coding_region_size_mb
#write.csv(samsum, "Startdata/TMB_per38.csv", row.names = F)

samsum <- read.csv("Startdata/TMB_per38.csv", header=T)
samsum <- samsum[substr(samsum$Tumor_Sample_Barcode, 14,15) == "01", ]
samsum$Tumor_Sample_Barcode <- sub("^([^-]*-[^-]*-[^-]*).*", "\\1", samsum$Tumor_Sample_Barcode)
samsum <- as.data.frame(samsum[,c(1,3)])


Scorestmb <- merge(Scorescl, samsum, by.x= "ID", by.y="Tumor_Sample_Barcode")

MRS <- read.csv("Results/MRS/GW_COAD_full.csv", header=T)
head(MRS)
MRS <- MRS[,c("X", "PICLORAM")]
Scorestmb <- merge(Scorestmb, MRS[,c("X", "PICLORAM")], by.x= "ID", by.y="X")
head(Scorestmb)
table(Scorestmb$mutCRC)

# Association between picloram MRS and young vs old tumors (SBS1) 
testtmb <- glm(mutCRC ~ PICLORAM + gender, family = binomial, data = Scorestmb) 
summary.glm(testtmb)

# Association between picloram MRS and young vs old tumors (SBS1) adjusted for TMB
testtmb <- glm(mutCRC ~ PICLORAM + gender + total_perMB, family = binomial, data = Scorestmb) 
summary.glm(testtmb)

# Association between picloram MRS and SBS1 score 
testtmb <- lm(SBS1 ~ PICLORAM + gender, data = Scorestmb) 
summary.lm(testtmb)

# Association between picloram MRS and TMB 
testtmb <- lm(total_perMB ~ PICLORAM + gender, data = Scorestmb) 
summary.lm(testtmb)


## Do we need to adjust for age at diagnosis when using the SBS1 clasification?

# Methylation risk scores are constructed as above using non-age-adjusted methylation data
# Load the non-age-adjusted methylation risk scores and clinical data (constructed as above)

clinic <-read.csv("Startdata/COAD_GW_MRS_notAdj.csv", header=T)

# Association between picloram MRS and young vs old tumors (SBS1) 
test <- glm(mutCRC ~ PICLORAM + gender, family = binomial, data = clinic) 
summary.glm(test)

# Association between picloram MRS and young vs old tumors (SBS1) adjusted for age at diagnosis 
test <- glm(mutCRC ~ PICLORAM + gender + age, family = binomial, data = clinic) 
summary.glm(test)
